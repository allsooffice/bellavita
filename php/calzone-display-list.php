...
echo '
      ...
      <ul>';
$pobierz_produkty = $mysqli->query("SELECT * FROM products WHERE category = 'calzone'");
while ($produkt=mysqli_fetch_array($pobierz_produkty) )
{
   ...
   <div class="pizza-list-cart"><div name="'.$produkt['id'].'" class="btn add-to-cart" size="" dough="calzone-dough" price="'.$produkt['price'].'"><i class=" icon-plus"></i>Zamawiam</div></div>
   </div>
